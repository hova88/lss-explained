...
const required = [
  ...
  "data/alignment.json", "data/lidar-frame.bin",
  ...
  ...["front-left", "front", "front-right", "back-left", "back", "back-right"].map(name=>`data/network-images/cam-${name}.jpg`),
];
...
const features=JSON.parse(await readFile(join(publicRoot,"data/model-features.json"),"utf8"));
const alignment=JSON.parse(await readFile(join(publicRoot,"data/alignment.json"),"utf8"));
if (rig.schema_version !== "2.0.0" || model.schema_version !== "2.0.0" || features.schema_version!=="2.0.0" || alignment.schema_version!=="2.0.0") throw new Error("Unexpected evidence schema");
