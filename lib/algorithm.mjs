...
export function reduceFeatureGroup(features, mode = "sum") {
  if (!features.length) return [];
  if (!new Set(["sum", "mean", "max"]).has(mode)) throw new Error(`Unsupported reduction: ${mode}`);
  const output = Array(features[0].length).fill(mode === "max" ? -Infinity : 0);
  features.forEach((feature) => feature.forEach((value, channel) => {
    output[channel] = mode === "max" ? Math.max(output[channel], value) : output[channel] + value;
  }));
  return mode === "mean" ? output.map((value) => value / features.length) : output;
}

export function bilinearSplatWeights(x, y) {
  const ix = Math.floor(x), iy = Math.floor(y), fx = x - ix, fy = y - iy;
  return [
    { index: [ix, iy], weight: (1 - fx) * (1 - fy) },
    { index: [ix + 1, iy], weight: fx * (1 - fy) },
    { index: [ix, iy + 1], weight: (1 - fx) * fy },
    { index: [ix + 1, iy + 1], weight: fx * fy },
  ];
}
